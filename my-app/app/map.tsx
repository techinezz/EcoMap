...
function MapClickHandler({
  ...
  const isPointInPolygons = useCallback(
    (point: L.LatLng): boolean => {
      if (!featureGroupRef.current) return false; // This check is why allowing null is safe
      ...
      featureGroupRef.current.eachLayer((layer) => {
        if (layer instanceof L.Polygon) {
          ...
          if (geoJsonCoords.length < 4) return;
  ...
  useMapEvents({
    click(e) {
      if (
        isTreeModeActive &&
        currentClusterCount < maxClusters &&
        isPointInPolygons(e.latlng)
      ) {
        ...
        for (let i = 0; i < treesToAddCount; i++) {
          ...
          individualTrees.push([
            e.latlng.lat + offsetY,
            e.latlng.lng + offsetX,
          ]);
        }
        onTreesPlaced({
...
const EcoMap: FC<EcoMapProps> = ({ targetLocation, onCoordinatesFinished }) => {
  ...
  const [currentCenter, setCurrentCenter] =
    useState<LatLngTuple>(newYorkCenter);
  ...
  const handleTreesPlaced = useCallback(
    ({
      ...
      setTreeClusters((prevClusters) => [
        ...prevClusters,
        { id: `cluster-${prevClusters.length}`, center, count },
      ]);
    },
    []
  );

  const handleMaxClustersReached = useCallback(() => {
  ...
  return (
    <div className="relative h-screen w-full">
      ...
      <div className="absolute top-[42px] left-[880px] z-[1000] flex flex-col items-end gap-2">
        ...
        {isTreeModeActive && (
          <div className="w-64 bg-white rounded-xl shadow-lg p-4 z-[1001]">
            <label
              htmlFor="brushSize"
              className="block text-sm font-medium text-[#25491B]"
            >
              Trees per click:{" "}
      ...
      {showMaxClusterAlert && (
        <div
          className="absolute top-10 left-1/2 transform -translate-x-1/2 z-[9999] 
      ...
      >
        ...
        <MapClickHandler
          ...
          onMaxClustersReached={handleMaxClustersReached}
        />
      </MapContainer>
    </div>
  );
